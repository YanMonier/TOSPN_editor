...
class PlacePropertyEditorTLSPN (QWidget):
	def __init__(self):
	...
	def update_name(self, new_name):
		...
		if self.current_place and new_name != self.current_place.name:
			if self.validate_name(new_name):
				self.current_place.TLSPN.rename_place(self.current_place, new_name)
			else:
				self.name_label.setText(f'Place name: {self.current_place.name}')

	def update_tokens(self, new_value):
		"""Update the number of tokens."""
		if self.current_place and new_value != self.current_place.init_token_number:
	...
	def validate_name(self, name):
		...
		for place in self.current_place.TLSPN.places.values():
	...
	def change_name(self, name):
		...
		dialog = ChangePlaceNameDialog(self.current_place.TLSPN, self.current_place)
		if dialog.exec_() == QDialog.Accepted:
			new_value = dialog.get_value()

			self.current_place.TLSPN.rename_place(self.current_place, new_value)
...
class ChangePlaceNameDialog(QDialog):
	...
	def __init__(self, TLSPN, placeItem=None):
		super().__init__()
		self.TLSPN = TLSPN
