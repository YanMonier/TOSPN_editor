...
class EventPropertyEditor(QWidget):
    ...
    def set_TOSPN(self, TOSPN):
        """Set the TOSPN model reference."""
        self.TOSPN = TOSPN
        self.reset_list()
        
        # Add existing events
        for event in self.TOSPN.events.values():
    ...
    def add_item(self):
        ...
        if not self.TOSPN:
            print("Debug: TOSPN is None")
            raise Exception("No TOSPN model set and opening add event dialog")
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if name and name != "λ":  # Don't allow adding lambda event
                ...
                eventObject = self.TOSPN.add_event(name)
    ...
    def remove_item(self, item, widget):
        ...
        if not widget.eventItem.transitions:  # Only remove if no transitions use this event
            self.TOSPN.remove_event(widget.eventItem)
    ...
    def edit_item(self, item, widget):
        ...
        dialog = AddEventDialog(self.TOSPN, eventItem)
        if dialog.exec_() == QDialog.Accepted:
            new_name = dialog.get_value()
            if new_name and new_name != "λ":  # Don't allow renaming to lambda
                self.TOSPN.rename_event(eventItem, new_name)
...
class AddEventDialog(QDialog):
    ...
    def validate_name(self, name):
        ...
        if name != self.initial_name and name in self.TOSPN.event_names:
            return False
