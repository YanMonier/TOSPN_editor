...
class GraphConstructionScene(QGraphicsScene):
	def __init__(self, parent=None):
		"""Initialize the scene for TOSPN construction."""
			...
			if self.state == "add_place":
				...
				if self.selected_item_1 is None:
					...
					for item in self.items_under_cursor:
						if isinstance(item, (GraphPlaceItem, GraphTransitionItem)):
		...
		if self.state == "add_arc":
			if self.selected_item_1 is not None:
				...
				for item in self.items_under_cursor:
					if isinstance(item, (GraphPlaceItem, GraphTransitionItem)):
						if item != self.selected_item_1:
							...
							if (isinstance(item, GraphPlaceItem) and 
								isinstance(self.selected_item_1, GraphTransitionItem)):
								self.selected_item_2 = item
								break
							elif (isinstance(item, GraphTransitionItem) and 
								  isinstance(self.selected_item_1, GraphPlaceItem)):
								self.selected_item_2 = item
								break
				...
				if self.selected_item_2 is not None:
					...
					# Get model objects
					source = self.selected_item_1.place if isinstance(self.selected_item_1, GraphPlaceItem) else self.selected_item_1.transition
					target = self.selected_item_2.place if isinstance(self.selected_item_2, GraphPlaceItem) else self.selected_item_2.transition
					
					# Create arc
					new_arc = self.TOSPN.add_arc(source, target)
	...
	def mouseMoveEvent(self, event):
		...
		if self.state in ["move", "add_arc", "add_place", "add_transition"]:
			...
			for item in self.items_under_cursor:
				if isinstance(item, (GraphPlaceItem, GraphTransitionItem, GraphArcItem, DraggableTextItem)):
		...
		if event.key() == Qt.Key_Delete:
			if self.state in ["move", "add_transition", "add_place"]:
				if self.mono_selected_item is not None:
					...
					elif isinstance(self.mono_selected_item, GraphTransitionItem):
						self.graphManager.remove_transition(self.mono_selected_item.transition)
					elif isinstance(self.mono_selected_item, GraphPlaceItem):
